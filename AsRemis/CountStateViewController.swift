...
class CountStateViewController: BaseViewController{
    ...
    func requestInformation(){
        ...
        http.listLiquidationDriver(idValue, completion: { (liquidationDriver) -> Void in
            self.totalPaymentLbl.text = "Pagamentos totais: \(liquidationDriver?.liquidation ?? "0.0")"
            self.currentAccountLbl.text = "Conta de saldo: \(liquidationDriver?.ingreso ?? "0.0")"
            self.totalToPayLbl.text = "Montante total a pagar: \(liquidationDriver?.egreso ?? "0.0")"
            self.totalCurrentValueLbl.text = "Montante total em favor: \(liquidationDriver?.ingresoProcess ?? "0.0")"
        })
    }
