...
class RequestServicesViewController: UIViewController, NVActivityIndicatorViewable {
    ...
    @IBAction func selectVehicleType(_ sender: Any) {
        ...
        for vehicle in optionsVehicles{
            arrTemp.append("Tipo de veículo: \(vehicle.vehiclenType ?? "")")
        }
    ...
    @IBAction func requestAction(_ sender: Any) {
        let userLocation = SingletonsObject.sharedInstance.userPosition
        ...
        http.fleetType({(fleets) -> Void in
            ...
            for fleet in fleets!{
                ...
                if self.fleetSelected.vehiclenType == ""{
                    ...
                    self.typeVehicleBtn.setTitle("Tipo de veículo: \(fleet.vehiclenType ?? "")", for: .normal)
                }
            }
        })
    }
...
extension RequestServicesViewController: AlertPickerDelegate{
    func indexSelected(_ index: Int, andTag: Int) {
        ...
        self.typeVehicleBtn.setTitle("Tipo de veículo: \(optionsVehicles[index].vehiclenType ?? "")", for: .normal)
    }
}
...
func showError(){
    let alertController = UIAlertController(title: "Erro", message: "Não foi possível registar a sua viagem, reveja as suas informações", preferredStyle: UIAlertControllerStyle.alert)
